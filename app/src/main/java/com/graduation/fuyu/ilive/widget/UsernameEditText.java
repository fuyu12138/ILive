 ...
 * login输入用户名
 * EditText控件
 * 不知道为什么getCompoundDrawablesRelative()获取的Drawable一直为空
 * 只好每个框都写一遍，唉。。。
...
public class UsernameEditText extends android.support.v7.widget.AppCompatEditText implements View.OnFocusChangeListener,TextWatcher {
    ...
    private void init() {
        ...
        if(mLeftDrawable==null){
            mLeftDrawable=ContextCompat.getDrawable(getContext(),R.drawable.ic_perm_identity_black_24dp);
        }
    ...
    public void onFocusChange(View view, boolean b) {
        if(b){
            mLeftDrawable=ContextCompat.getDrawable(getContext(),R.drawable.ic_perm_identity_pink_24dp);
            mLeftDrawable.setBounds(0,0,50,50);
